...
# Import the local open-source LLM (Ollama)
...
def upload_pdf():
    ...
    try:
        ...
        vectorstore_builder.build_and_save_opensource(chunks, 'faiss_index_opensource') 
        user_chains.clear()
...
def ask_question():
    """
    Receive user question, pass it to the open-source conversation chain, and return the answer.
    """
    ...
    try:
        if user_id not in user_chains:
            vectorstore = vectorstore_builder.load_vectorstore('faiss_index_opensource', use_openai=False)
